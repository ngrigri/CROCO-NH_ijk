...
!-
    ...
    if (south.ne.MPI_PROC_NULL) then
       ...
       do ih = 1, nh
          if (ih == 1) then
             a2D(1:nx,0) = a2D(1:nx,1)
          elseif(ih == 2) then
             a2D(1:nx,-1) = 2._rp * a2D(1:nx,1) - a2D(1:nx,2)
          else
             stop
          endif
       enddo
    ...
    if (north.ne.MPI_PROC_NULL) then
       ...
       do ih = 1, nh
          if (ih == 1) then
             a2D(1:nx,ny+1) = a2D(1:nx,ny)
          elseif(ih == 2) then
             a2D(1:nx,ny+2) = 2._rp * a2D(1:nx,ny) - a2D(1:nx,ny-1)
          else
             stop
          endif
       enddo
    ...
    if (west.ne.MPI_PROC_NULL) then
       ...
       do ih = 1, nh
          if (ih == 1) then
             a2D(0,1:ny) = a2D(1,1:ny)
          elseif(ih == 2) then
             a2D(-1,1:ny) = 2._rp * a2D(1,1:ny) - a2D(2,1:ny)
          else
             stop
          endif
       enddo
    ...
    if (nh==2) then
       ...
       elseif (east.ne.MPI_PROC_NULL) then
          ...
          a2D(nx+1:nx+nh,1-nh:0) = a2D(nx:nx-nh+1:-1,nh:1:-1)
       endif
       ...
       elseif (east.ne.MPI_PROC_NULL) then
          ...
          a2D(nx+1:nx+nh,ny+1:ny+nh) = a2D(nx:nx-nh+1:-1,ny:ny-nh+1:-1)
       endif
       ...
       elseif (west.ne.MPI_PROC_NULL) then
          ...
          a2D(1-nh:0,ny+1:ny+nh) = a2D(nh:1:-1,ny:ny-nh+1:-1)
       endif
    ...
    if (nh==2) then
       ...
       if (northeast.ne.MPI_PROC_NULL) then
          sendNE(:,:) = a2D(nx-nh+1:nx,ny-nh+1:ny) 
          call MPI_ISend(                                   &
    ...
    if (nh==2) then
       ...
       if (flag_se_s) then
          a2D(nx+1:nx+nh,1-nh:0) = a2D(nx:nx-nh+1:-1,1-nh:0)
       elseif (flag_se_e) then
          a2D(nx+1:nx+nh,1-nh:0) = a2D(nx+1:nx+nh,nh:1:-1)
       endif

       if (flag_ne_n) then
          a2D(nx+1:nx+nh,ny+1:ny+nh) = a2D(nx:nx-nh+1:-1,ny+1:ny+nh)
       elseif (flag_ne_e) then
          a2D(nx+1:nx+nh,ny+1:ny+nh) = a2D(nx+1:nx+nh,ny:ny-nh+1:-1)
       endif

       if (flag_nw_n) then
          a2D(1-nh:0,ny+1:ny+nh) = a2D(nh:1:-1,ny+1:ny+nh)
       elseif (flag_nw_w) then
          a2D(1-nh:0,ny+1:ny+nh) = a2D(1-nh:0,ny:ny-nh+1:-1)
       endif
    endif
  ...
  subroutine fill_halo_3D(lev,p,lbc_null)
    ...
    elseif ((lbc).and.(trim(cuv)=='u')) then
       ...
       p(nx+1:nx+nh,1:ny,:) = p(nx:nx-nh+1:-1,1:ny,:)
    endif
    ...
    elseif ((lbc).and.(trim(cuv)=='v')) then 
       ...
       p(1:nx,ny+1:ny+nh,:) = p(1:nx,ny:ny-nh+1:-1,:)
    endif
    ...
    elseif ((lbc).and.(east.eq.MPI_PROC_NULL).and.(trim(cuv)=='u')) then
       p(nx+1:nx+nh,1-nh:0,:) = 0._8
    elseif (south.ne.MPI_PROC_NULL) then
       flag_se_s = .true.
    elseif (east.ne.MPI_PROC_NULL) then
       flag_se_e = .true.
    else!!Homogenous Neumann  
       p(nx+1:nx+nh,1-nh:0,:) = p(nx:nx-nh+1:-1,nh:1:-1,:)
    endif
    ...
    elseif (((lbc).and.(east.eq.MPI_PROC_NULL).and.(trim(cuv)=='u')).or. &
       ...
       p(nx+1:nx+nh,ny+1:ny+nh,:) = 0._8
    elseif (north.ne.MPI_PROC_NULL) then
       flag_ne_n = .true.
    elseif (east.ne.MPI_PROC_NULL) then
       flag_ne_e = .true.
    else!!Homogenous Neumann  
       p(nx+1:nx+nh,ny+1:ny+nh,:) = p(nx:nx-nh+1:-1,ny:ny-nh+1:-1,:)
    endif
    ...
    elseif ( ((lbc).and.(west.eq.MPI_PROC_NULL).and.(trim(cuv)=='u')).or. &
         ((lbc).and.(trim(cuv)=='v')) ) then
       p(1-nh:0,ny+1:ny+nh,:) = 0._8
    elseif (north.ne.MPI_PROC_NULL) then
       flag_nw_n = .true.
    elseif (west.ne.MPI_PROC_NULL) then
       flag_nw_w = .true.
    else !!Homogenous Neumann  
       p(1-nh:0,ny+1:ny+nh,:) = p(nh:1:-1,ny:ny-nh+1:-1,:)
    endif
    ...
    if (flag_se_s) then
       p(nx+1:nx+nh,1-nh:0,:) = p(nx:nx-nh+1:-1,1-nh:0,:)
    elseif (flag_se_e) then
       p(nx+1:nx+nh,1-nh:0,:) = p(nx+1:nx+nh,nh:1:-1,:)
    endif

    if (flag_ne_n) then
       p(nx+1:nx+nh,ny+1:ny+nh,:) = p(nx:nx-nh+1:-1,ny+1:ny+nh,:)
    elseif (flag_ne_e) then
       p(nx+1:nx+nh,ny+1:ny+nh,:) = p(nx+1:nx+nh,ny:ny-nh+1:-1,:)
    endif

    if (flag_nw_n) then
       p(1-nh:0,ny+1:ny+nh,:) = p(nh:1:-1,ny+1:ny+nh,:)
    elseif (flag_nw_w) then
       p(1-nh:0,ny+1:ny+nh,:) = p(1-nh:0,ny:ny-nh+1:-1,:)
    endif
