...
contains
  ...
  subroutine define_grids(npxg, npyg, nxl, nyl, nzl)
    ...
    do lev=1,nlevs
       ...
       allocate(grid(lev)%zeta(-1:nx+2,-1:ny+2     ))
       allocate(grid(lev)%h   (-1:nx+2,-1:ny+2     ))
       allocate(grid(lev)%zr  (-1:nx+2,-1:ny+2,nz  ))
       allocate(grid(lev)%zw  (-1:nx+2,-1:ny+2,nz+1))
       allocate(grid(lev)%cw  ( 0:nx+1, 0:ny+1,nz+1))
    enddo
    ...
    ny = grid(lev)%ny
    nz = grid(lev)%nz
    allocate(grid(lev)%dzw( 0:nx+1,0:ny+1,nz+1))
    allocate(grid(lev)%zxdy(0:nx+1,0:ny+1,nz ))
    allocate(grid(lev)%zydx(0:nx+1,0:ny+1,nz  ))

    allocate(grid(lev)%dummy3Dnz( 0:nx+1,0:ny+1,nz  ))
    allocate(grid(lev)%dummy3Dnzp(0:nx+1,0:ny+1,nz+1))
 
    do lev=1,nlevs
       nx = grid(lev)%nx
       ny = grid(lev)%ny
       nz = grid(lev)%nz
       allocate(grid(lev)%p(0:nx+1,0:ny+1,nz))
       allocate(grid(lev)%b(0:nx+1,0:ny+1,nz))
       allocate(grid(lev)%r(0:nx+1,0:ny+1,nz))
    enddo
    ...
    do lev=1,nlevs
       ...
       allocate(grid(lev)%cA(nd,0:nx+1,0:ny+1,nz))
    enddo

    do lev=1,nlevs
       nx = grid(lev)%nx
       ny = grid(lev)%ny
       allocate(grid(lev)%dx(0:nx+1,0:ny+1))
       allocate(grid(lev)%dy(0:nx+1,0:ny+1))
    enddo

    ! MPI exhanges for 2D arrays
    do lev=1,nlevs
       nx = grid(lev)%nx
       allocate(gbuffers(lev)%sendS2D1(nx,1))
       allocate(gbuffers(lev)%recvS2D1(nx,1))
       allocate(gbuffers(lev)%sendN2D1(nx,1))
       allocate(gbuffers(lev)%recvN2D1(nx,1))
    enddo

    do lev=1,nlevs
       ny = grid(lev)%ny
       allocate(gbuffers(lev)%sendE2D1(1,ny))
       allocate(gbuffers(lev)%recvE2D1(1,ny))
       allocate(gbuffers(lev)%sendW2D1(1,ny))
       allocate(gbuffers(lev)%recvW2D1(1,ny))
    enddo

    do lev=1,nlevs
       nx = grid(lev)%nx
       allocate(gbuffers(lev)%sendS2D2(nx,2))
       allocate(gbuffers(lev)%recvS2D2(nx,2))
       allocate(gbuffers(lev)%sendN2D2(nx,2))
       allocate(gbuffers(lev)%recvN2D2(nx,2))
    enddo

    do lev=1,nlevs
       ny = grid(lev)%ny
       allocate(gbuffers(lev)%sendE2D2(2,ny))
       allocate(gbuffers(lev)%recvE2D2(2,ny))
       allocate(gbuffers(lev)%sendW2D2(2,ny))
       allocate(gbuffers(lev)%recvW2D2(2,ny))
    enddo
    ...
    do lev=1,nlevs
       nx = grid(lev)%nx
       nz = grid(lev)%nz
       allocate(gbuffers(lev)%sendS(nx,nz))
       allocate(gbuffers(lev)%recvS(nx,nz))
       allocate(gbuffers(lev)%sendN(nx,nz))
       allocate(gbuffers(lev)%recvN(nx,nz))
    enddo

    do lev=1,nlevs
       ny = grid(lev)%ny
       nz = grid(lev)%nz
       allocate(gbuffers(lev)%sendE(ny,nz))
       allocate(gbuffers(lev)%recvE(ny,nz))
       allocate(gbuffers(lev)%sendW(ny,nz))
       allocate(gbuffers(lev)%recvW(ny,nz))
    enddo
    ...
    do lev=1,nlevs
       nx = grid(lev)%nx
       nz = grid(lev)%nz
       allocate(gbuffers(lev)%sendSp(nx,nz+1))
       allocate(gbuffers(lev)%recvSp(nx,nz+1))
       allocate(gbuffers(lev)%sendNp(nx,nz+1))
       allocate(gbuffers(lev)%recvNp(nx,nz+1))
    enddo

    do lev=1,nlevs
       ny = grid(lev)%ny
       nz = grid(lev)%nz
       allocate(gbuffers(lev)%sendEp(ny,nz+1))
       allocate(gbuffers(lev)%recvEp(ny,nz+1))
       allocate(gbuffers(lev)%sendWp(ny,nz+1))
       allocate(gbuffers(lev)%recvWp(ny,nz+1))
    enddo
    ...
    do lev=1,nlevs
       ...
       allocate(gbuffers(lev)%sendS4D(nd,nx,nz))
       allocate(gbuffers(lev)%recvS4D(nd,nx,nz))
       allocate(gbuffers(lev)%sendN4D(nd,nx,nz))
       allocate(gbuffers(lev)%recvN4D(nd,nx,nz))
    enddo
    ...
    do lev=1,nlevs
       ...
       allocate(gbuffers(lev)%sendE4D(nd,ny,nz))
       allocate(gbuffers(lev)%recvE4D(nd,ny,nz))
       allocate(gbuffers(lev)%sendW4D(nd,ny,nz))
       allocate(gbuffers(lev)%recvW4D(nd,ny,nz))
    enddo
