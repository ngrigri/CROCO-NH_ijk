...
!       if(myrank == 0) write(*,*)" *** after gather **** b(1,1,1)=",grid(lev+1)%b(1,1,1)
  ...
  subroutine fine2coarse_2D(x,y,nx,ny)
    ...
    if(d==1)then
       ...
       do j2=1,ny
          ...
          do i2=1,nx
             i=2*i2-1
             y(i2,j2,1) = (x(i,j,1)+x(i+1,j,1)+x(i,j+1,1)+x(i+1,j+1,1))
          enddo
       enddo
    else
       ! x was 3D
       do j2=1,ny
          ...
          do i2=1,nx
             ...
             y(i2,j2,1) = &
                  ...
                  +x(i,j,2)+x(i+1,j,2)+x(i,j+1,2)+x(i+1,j+1,2))
          enddo
       enddo
    endif
    ...
    if(d==1)then
       ...
       do j2=1,ny
          ...
          do i2=1,nx
             i=2*i2-1
             xf(i  ,j  ,1) = xc(i2,j2,1)
             xf(i  ,j+1,1) = xc(i2,j2,1)
             xf(i+1,j  ,1) = xc(i2,j2,1)
             xf(i+1,j+1,1) = xc(i2,j2,1)
          enddo
       enddo
    else
       ! xf is 3D
       do j2=1,ny
          ...
          do i2=1,nx
             ...
             xf(i  ,j  ,2) = xc(i2,j2,1)
             xf(i  ,j+1,1) = xc(i2,j2,1)
             xf(i  ,j+1,2) = xc(i2,j2,1)
             xf(i+1,j  ,1) = xc(i2,j2,1)
             xf(i+1,j  ,2) = xc(i2,j2,1)
             xf(i+1,j+1,1) = xc(i2,j2,1)
             xf(i+1,j+1,2) = xc(i2,j2,1)
          enddo
       enddo
    endif
  ...
  subroutine coarse2fine_2D_linear(xf,xc,nx,ny)
    ...
    do j2=1,ny
       ...
       do i2=1,nx
          i=2*i2-1
          xf(i,j,k) =  &
               + a * xc(i2,j2  ,k2) + c * xc(i2-1,j2-1,k2) &
               + b * xc(i2,j2-1,k2) + b * xc(i2-1,j2  ,k2)
          xf(i,j+1,k) =  &
               + a * xc(i2,j2  ,k2) + c * xc(i2-1,j2+1,k2) &
               + b * xc(i2,j2+1,k2) + b * xc(i2-1,j2  ,k2)
          xf(i+1,j,k) =  &
  ...
  subroutine coarse2fine_3D_nearest(xf,xc,nx,ny,nz)
    ...
    do k2=1,nz
       ...
       do j2=1,ny
          ...
          do i2=1,nx
             i=2*i2-1
             xf(i  ,j  ,k  ) = xc(i2,j2,k2)
             xf(i  ,j  ,k+1) = xc(i2,j2,k2)
             xf(i  ,j+1,k  ) = xc(i2,j2,k2)
             xf(i  ,j+1,k+1) = xc(i2,j2,k2)
             xf(i+1,j  ,k  ) = xc(i2,j2,k2)
             xf(i+1,j  ,k+1) = xc(i2,j2,k2)
             xf(i+1,j+1,k  ) = xc(i2,j2,k2)
             xf(i+1,j+1,k+1) = xc(i2,j2,k2)
          enddo
       enddo
    enddo
  ...
  subroutine coarse2fine_3D_linear(xf,xc,nx,ny,nz)
    ...
    ! interior level
    do k=2,nz*2-1
       k2 = ((k+1)/2)          ! kp = k2+1 for k=2,4 ..
       kp = k2-(mod(k,2)*2-1)  ! kp = k2-1 for k=3,5 ..
       do j2=1,ny
          j=2*j2-1
          do i2=1,nx
             i=2*i2-1
             xf(i  ,j  ,k  ) =  &
                  + d * xc(i2,j2  ,k2) + f * xc(i2-1,j2-1,k2) &
                  + e * xc(i2,j2-1,k2) + e * xc(i2-1,j2  ,k2) &
                  + e * xc(i2,j2  ,kp) + g * xc(i2-1,j2-1,kp) &
                  + f * xc(i2,j2-1,kp) + f * xc(i2-1,j2  ,kp)
             xf(i  ,j+1,k  ) =  &
                  + d * xc(i2,j2  ,k2) + f * xc(i2-1,j2+1,k2) &
                  + e * xc(i2,j2+1,k2) + e * xc(i2-1,j2  ,k2) &
                  + e * xc(i2,j2  ,kp) + g * xc(i2-1,j2+1,kp) &
